...
public class UserPannel extends javax.swing.JFrame {
    ...
    public UserPannel() {
        ...
        List<User> users = Controller.Helper.searchUsers("eq", "account_id", userID);
        User user = users.get(0);
        userBalance = user.getBalance();
        balance_lb.setText(String.valueOf(userBalance + " $"));
        info_id.setText(user.getAccount_id());
        info_name.setText(user.getName());
        info_gender.setText(user.getGender());
        info_phone.setText(user.getPhone());
        info_email.setText(user.getEmail());
        info_balance.setText(user.getBalance() + " $");
        ...
        if (ammountInput != null) {
            if (ammountInput.matches("[0-9]*([.]{1})?[0-9]{1,2}")) {
                try {
                    ...
                    long flag = Controller.Helper.updateUser("eq", "account_id", userID, "balance", ammount);

                    List<User> users = Controller.Helper.searchUsers("eq", "account_id", userID);
                    User user = users.get(0);
                    userBalance = user.getBalance();
                    balance_lb.setText(String.valueOf(user.getBalance() + " $"));
                    info_balance.setText(user.getBalance() + " $");
                    if (flag >= 1) {
    ...
    private void save_btnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_save_btnActionPerformed
        try {
            if (info_name.getText().equals("")
                ...
                long flag1 = Controller.Helper.updateUser("eq", "account_id", userID, "name", info_name.getText());
                long flag2 = Controller.Helper.updateUser("eq", "account_id", userID, "phone", info_phone.getText());
                long flag3 = Controller.Helper.updateUser("eq", "account_id", userID, "email", info_email.getText());
            ...
            if (ammountInput != null) {
                if (ammountInput.matches("[0-9]*([.]{1})?[0-9]{1,2}")) {
                    try {
                        ...
                        List<User> users = Controller.Helper.searchUsers("eq", "account_id", userID);
                        User user = users.get(0);
                        double balanceFrom = user.getBalance();

                        double ammount = Double.parseDouble(ammountInput);
                        if (ammount <= balanceFrom) {

                            List<User> usersTo = Controller.Helper.searchUsers("eq", "account_id", account_to_tf.getText());

                            if (!(usersTo.isEmpty())) {
                                User userTo = usersTo.get(0);
                                double balanceTo = userTo.getBalance();

                                ammount = Math.round(ammount * 100.0) / 100.0;
                                balanceFrom -= ammount;
                                balanceFrom = Math.round(balanceFrom * 100.0) / 100.0;
                                balanceTo += ammount;
                                userBalance = balanceFrom;

                                long flag1 = Controller.Helper.updateUser("eq", "account_id", userID, "balance", balanceFrom);
                                long flag2 = Controller.Helper.updateUser("eq", "account_id", account_to_tf.getText(), "balance", balanceTo);
    ...
    private void save_password_btnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_save_password_btnActionPerformed
        try {
            ...
            if (old_password_tf.getText().equals("")
                ...
                byte[] messageDigest2 = md.digest(newPassword.getBytes());
                BigInteger number2 = new BigInteger(1, messageDigest2);
                String newPasswordHashed = number2.toString(16);

                long flag = Controller.Helper.updateUser("eq", "account_id", userID, "password", newPasswordHashed);
